...
opt_out_data <- function(x) {
  ...
  here("data", y, paste0(y, "_opt-outs-anon.rds")) %>%
    read_rds() %>%
...
reg <-
  here("data", "registration-data", paste0(wave, "_registration-data.rds")) %>%
  read_rds() %>%
...
latest %>%
  group_by(age_group) %>%
  summarise_at(vars(n_resp, n_reg), sum) %>%
  ungroup() %>%
...
demog_data <- function(x) {
  ...
  here("data", y, paste0(y, "_registration-data-anon.rds")) %>%
    read_rds() %>%
...
trend <- map_dfr(c(31:42, 44:wave), demog_data)
...
reg <-
  list.files(here("data", "registration-data"), full.names = TRUE) %>%
  map_dfr(
    ~ read_rds(.x) %>% 
      filter(status == "active") %>%
      mutate(age_group = age(date_of_birth, wave, grouped = TRUE)) %>%
      ...
      summarise(n_reg = n()) %>%
      ungroup() %>%
      mutate(wave = as.numeric(str_extract(.x, "\\d{2}")), 
             date = start_date(wave)) %>%
...
opt_outs <- map_dfr(c(31:42, 44:wave), opt_out_data)
